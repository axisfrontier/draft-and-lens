1. Apply the proposed ledger empty state copy — approved as written:

"I haven't started tracking anything here yet.
I collect facts as I read complete chapters. I take nothing from an excerpt — a draft mid-revision isn't something to hold the rest of the book to."

2. Add the editor voice standard to CLAUDE.md as a standing rule: first person throughout, developmental, no product-speak. Include the four deliberate exceptions: legal statement ("We never train"), dev-facing 400 errors, fragment options written in the writer's first person, and stage labels. This must survive future sessions.

3. Create DraftAndLens_DepthAndScenarios_Spec.md in the repo root with exactly this content — do not implement anything in it yet, commit as "Add depth and scenarios spec":
...
4. Add a pointer to DL_ONLY_ReadFirst.md under file pointers:
...
Then begin building per the spec's build order: tradition depth gap first (Brain 2 extension, no new storage), scenarios page in parallel. Flag before building anything that requires a Supabase migration. Resume automatically through any usage limits.
