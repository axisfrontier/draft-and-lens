...
- Providers: Anthropic (reading generation), Supabase (storage, EU/UK region), Clerk (auth), Google (optional sign-in, via Clerk), Vercel (hosting), Stripe (billing, when live).
...
## Who we are
Draft & Lens ("D&L", "we", "us") provides an AI-generated literary reading service. For data-protection purposes, the data controller is [YOUR LEGAL NAME / COMPANY NAME], contactable at hello@draftandlens.com. [If registered as a company: company number [X], registered at [ADDRESS].]
...
## What we collect
- **Your account email**, used to create and access your account. We do not require your real name. If you sign in with Google, Google shares your email address and basic profile information (name, profile photo) with us via our authentication provider, Clerk, for this purpose only.
...
- It is sent to **Anthropic** (our AI provider) to generate your reading. Anthropic's terms state it does not use API data to train its models, and does not retain your submitted text or the generated reading by default.
- **The one exception, stated plainly:** if a submission is flagged by Anthropic's own automated safety systems, Anthropic may retain it for **up to 2 years** under their policy. Because D&L deliberately welcomes serious literary work that engages with dark, violent, or sexual subject matter, a false-positive safety flag on legitimate fiction is a real possibility for exactly the writers this service is built for — not a remote edge case. *(Claim reflects Anthropic's data-retention terms as checked 26 July 2026; see Anthropic's current terms for any changes since.)*
...
- **Google** — if you choose "Sign in with Google," Google authenticates you and shares your email and basic profile information with us via Clerk. We do not receive any other Google account data.
...
To exercise any right not available in-app, contact hello@draftandlens.com.
...
## Contact
Questions about this policy: hello@draftandlens.com
...
- Confirm **processor list and DPAs** are complete and current, including Google (added as a sign-in option 2026-07-25 — confirm whether Google's own DPA terms for this use are sufficient or a separate agreement is needed).
- **Specifically want your view on:** Anthropic's up-to-2-year retention on safety-flagged submissions (checked 26 July 2026, see the Privacy Policy section above) set against D&L's deliberate policy of welcoming dark, violent, or sexual literary content right up to the Acceptable Use boundary. A false-positive flag on serious fiction is a foreseeable scenario, not a hypothetical — want to know whether that combination needs disclosure beyond what's currently drafted, or any mitigation.
