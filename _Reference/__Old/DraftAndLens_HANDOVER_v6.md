...
- **BLOCKED:** Nenad hit Cursor's **free-tier monthly limit** mid-Stage-1. Resets ~**30 June 2026**. Also: free tier locks the model to **Composer 2.5 Fast** (weakest; no Claude/premium). The migration is security-critical, so the weak model is a real risk — verify its work carefully.

### THE IMMEDIATE NEXT ACTION (when Cursor resets)
1. In Cursor, ask it: *"Run npm install, then npm test, then npm run dev. Tell me whether the test passed and the local URL."*
2. Confirm `npm test` = **pass**.
3. **Run the browser security check** (the whole point of Stage 1): open the local URL in Chrome → right-click → Inspect → **Sources** tab → Cmd+Option+F (search all) → search these five, one at a time, each must return **ZERO**:
...
4. All zero → approve **Stage 2**. Any hit → tell Cursor *"This is still reachable in the client bundle; it must be server-only. Fix it,"* re-check.
...
- `DraftAndLens_HANDOVER_v6.md` — this file
...
**The 5 files to put in Cursor's folder (nothing else):** `DraftAndLens.html`, `DraftAndLens_Architecture_v6.md`, `DraftAndLens_CursorPrompt_v6.md`, `DraftAndLens_LearnedCorpus.md`, `ThinkingDiscipline.md`.
...
This handover was written at the end of a very long chat (which had become slow). When continuing in a fresh conversation: the production *decisions* persist via memory; this file holds the *state*. Re-upload it (and the current HTML if doing build work) at the start of the new chat. The next concrete action is unchanged: **wait for Cursor's free reset (~30 June), re-upload the latest HTML, run the Stage-1 browser security check, then approve Stage 2.**

---

*Draft & Lens — Handover v6 · June 2026 · pairs with Architecture v6.0*
