...
7. **Submission/Match agent ("Scriptmatch") — POST-LAUNCH, separate product surface, logged not specced.** Idea: once a work is finished, an agent searches for who is seeking scripts/books (agents, producers, publishers, open calls), tells the writer, drafts an outreach note, shows it to the writer, and sends it on approval. Genuinely valuable and fits the "help the writer, don't replace them" ethos — BUT two hard constraints: (a) it is a market-facing surface, NOT editorial — it belongs well after the core read ships, never pulling focus from finishing D&L's reading; (b) it drafts the writer's *pitch*, which brushes the founding "never generates the writer's words" line — so it must be **draft-and-approve only, never auto-send**, framed exactly like the illustrative sampler (an option the writer edits and owns, not ghostwritten self-promotion). Do not build until the core product is launched and proven.
...
## 10 · EXPERT VIEW (spec written this session — `DraftAndLens_ExpertMode_Spec.md`)

A full design spec now exists for an Expert/advanced reading toggle. Core decisions, so a fresh chat doesn't re-derive them:

- **The principle:** Expert View is the **same reading, less translated** — NOT smarter, longer, or harsher. Default mode teaches craft terms as it goes; Expert View assumes them and reinvests the words in depth (precise tradition placement, best-in-class anchoring, visible disagreement, diagnostic confidence). The underlying analysis is identical — same brains, same pipeline.
- **Framing is load-bearing:** "translated vs. untranslated," NEVER "beginner vs. expert." No hierarchy, no tier-shaming; the default is the full reading, just explained. Expert View is "a place to grow into, not a gate."
- **Answers the founder's worry** (an amateur feeling locked out) with three non-blocking layers: honest toggle microcopy + a first-time intro panel + live inline glossary links on every craft term (reuses the §19 glossary). So an amateur *can* read Expert View — they tap terms they don't know. It becomes a teaching surface.
- **Key behaviour:** discussion + sampler, not verdict — explain the *underlying* problem and show a direction (per LearnedCorpus v2.3 illustrative examples), never a full rewrite. Worked example in the spec: "not minimalism in general — Carver-via-Lish vs. later-Carver," with the named terms glossary-linked to unfold the Lish editing history. This is the thesis: maximal precision on the surface, depth one tap beneath.
- **Discipline:** denser and braver, NEVER longer (no extra nitpicks); architecture over surface; never an external rubric; the reading underneath must NOT be a separate harsher pipeline.
- **Sync check done:** maps cleanly onto the default view — no contradictions, because Expert View re-presents the same pipeline output. Two things flagged for build: (1) whether the depth deltas already exist in the analyst output and are *revealed*, or need a register parameter (audit analyst output first; either way NOT a second pipeline); (2) the interrogation register ("was this ambition worth attempting?") is sharper than the default's confidence-preserving tone — acceptable because it's opt-in within an opt-in mode, but a deliberate choice to confirm.
- **Open:** tier placement (likely free — it's a presentation preference, not a capability gate, and the free tier is never hobbled); per-reading toggle vs persistent preference (likely both).

Status: spec only, **post-Stage-E**. Build after the core report UI exists.

---

## 9 · CONVERSATION-STATE NOTE

This handover holds the *state*; decisions also persist via memory. Re-upload it (and the current HTML if doing build work) at the start of a new chat. The next concrete action: **the migration works end to end (proven by live readings). Apply the title change (verdict→rewrite), then build Stage E — the full report UI (scores/radar, story arc, market, bible, partial-read banner, inline anchoring §18). Audit the existing components (RadarChart, StoryArc, AnchoredView, etc.) for built-vs-stub before writing anything new. Keep the IP boundary intact and re-run the browser security check after, since new UI is new client surface.** Expert View (§10) is specced but post-Stage-E. Scriptmatch (§5.7) is post-launch.

---

*Draft & Lens — Handover v8 · June 2026 · pairs with Architecture v6.0, LearnedCorpus v2.3, and ExpertMode_Spec*
