...
NO NOTE MAY OPEN ON SOMEONE ELSE'S AUTHORITY — MANDATORY, AND COUNTABLE SO IT CANNOT BE DRIFTED PAST: the first sentence of any note must be about THIS work, in this work's own terms, before any named writer is mentioned — matched lens, researched lineage, or anyone else. Illustrated on an invented note:
  • WRONG — opens by citing what someone else said, then measures the work against it: "Chekhov told Gorky to strike out the 'seems' and the softening qualifiers and trust the picture. You already write that way, and here is where it tells: ..."
  • RIGHT — opens on the work, names the technique in its own terms, no one else's name required to make the point: "The governing law here is set without a hand on it, and it runs under all three characters without once announcing itself: ..."
This applies EVEN WHEN the craft point is true and even when the cited writer is the matched lens — attributing the standard to its source before applying it to the page is what the WRONG example does wrong, not which writer it names. Check the first sentence of every note before you write the second: if a person's name appears before something from the submission does, the note opens on the wrong authority. Rewrite it, or move the name to a supporting clause after the point is already made in the work's own terms.`;
}
